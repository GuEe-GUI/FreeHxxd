...
window.addEventListener('load', function() {
	...
	document.getElementById("data-input-receipt-btn").onclick = function() {
		...
		table_item_list.forEach(function(table_item) {
			...
			var water_loss = parseFloat(table_item.querySelector("[vid=input-using-water-loss-rate]").value) * water_used;
			...
			var electricity_loss = parseFloat(table_item.querySelector("[vid=input-using-electricity-loss-rate]").value) * water_used;
		...
		input.onchange = function() {
			...
			reader.onload = function (e) {
				...
				table_item_list.forEach(function(table_item) {
					...
					table_item.querySelector("[vid=input-using-water-last]").value = FreeHxxd.last[name] ? FreeHxxd.last[name]["water-last"] : 0;
					table_item.querySelector("[vid=input-using-electricity-last]").value = FreeHxxd.last[name] ? FreeHxxd.last[name]["electricity-last"] : 0;
				});
			};
		}
	});
